...
router.get('/', async (req, res) => {
  try {
    console.log('🔍 Admin Service: Fetching bookings (no auth)...');
    ...
    // Make request to customer service for bookings - NO auth required
...
// Get booking statistics (no authentication required)
router.get('/statistics', async (req, res) => {
  try {
    console.log('🔍 Admin Service: Fetching booking statistics (no auth)...');
    ...
    if (error.response) {
      res.status(error.response.status).json({
        success: false,
        message: error.response.data?.message || 'Failed to fetch booking statistics from customer service'
      });
    } else if (error.code === 'ECONNREFUSED') {
      res.status(503).json({
        success: false,
...
// Get booking by ID (no authentication required)
router.get('/:id', async (req, res) => {
  try {
    ...
    if (error.response) {
      res.status(error.response.status).json({
        success: false,
        message: error.response.data?.message || 'Failed to fetch booking from customer service'
      });
    } else if (error.code === 'ECONNREFUSED') {
      res.status(503).json({
        success: false,
...
// Update booking status (no authentication required)
router.put('/:id/status', async (req, res) => {
  try {
    ...
    if (error.response) {
      res.status(error.response.status).json({
        success: false,
        message: error.response.data?.message || 'Failed to update booking status in customer service'
      });
    } else if (error.code === 'ECONNREFUSED') {
      res.status(503).json({
        success: false,
